...
for item in (root / "java").iterdir():
    java_versions.append(item.name)
...
nodejs_versions.sort(key=lambda x: VersionInfo.parse(x), reverse=True)
java_versions.sort(key=lambda x: VersionInfo.parse(x), reverse=True)
